...
FI.means[FI.means$Type=='Food_source','Type'] <- 'Food source'
...
#ggsave(plot=Fig1,filename='Figure_1.eps',unit='cm',height=16,width=16,dpi=1000)
...
dynamics.plot <- function(variable,variable.name,rm=F) {
  ...
  if(rm==T){              ## Just to harmonize colours and shapes
    Dynamics <- Dynamics + 
      scale_colour_manual(values=c("#B79F00", "#619CFF", "#00BA38", "#00BFC4" )) +
...
dynamics.plot("Stachyose","Stachyose",rm=T)+ggtitle('Stachyose')
...
#ggsave(Fig2,filename = 'Figure_2.eps',units='cm',dpi=1000,width=16,height=12)
...
PO.melt <- melt(insects.data[,c(2:4,6:15,16)],id.vars=c('Parasitoid_Origin','Sugar_treatment','Hour_treatment','Tmt.12'))
