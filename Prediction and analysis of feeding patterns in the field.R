...
source('Functions for adjusted counting.R', encoding = 'UTF-8')
...
Fig3
  ...
  #ggsave(Fig3,filename='Figure_3.eps',units='cm',dpi=1000,width=16,height=10)
...
Fig4 <- ggplot(tabAdj , aes(Position, fill = Feeding_class))+
  ...
  ylab('Percentage of individuals from each feeding class')+
  ...
  scale_y_continuous(limits = c(0, NA),
                     expand = expansion(mult = c(0, 0.02)))+
  geom_vline(data = data.frame(xint=0.4,Type="Intercrop"), aes(xintercept = xint))

Fig4

#ggsave(Fig4,filename='Figure_4.png',height=14,width=16, unit='cm',dpi=1000)
#ggsave(Fig4,filename='Figure_4.eps',height=14,width=16, unit='cm',dpi=1000)
...
FigS4_1 <- ggplot(field, aes(y=TotSug, x= Field,fill=Field_type)) +
  facet_wrap(~Field_position)+
  geom_boxplot(alpha=0.6,outlier.shape = NA)+
  geom_point(aes(colour=Field_type),alpha=1,size=1.5,
             position = position_jitterdodge())+
  ...
  scale_y_continuous(limits = c(0, NA),
                     expand = expansion(mult = c(0, 0.02)))+
  geom_vline(data = filter(field, Field_position=="5m"), aes(xintercept = 0.4))

#ggsave(FigS4_1,filename='Figure_S4_1.png',height=12,width=16, unit='cm',dpi=1000)
